...
public class GeneralController {
	...
	@PostMapping("/login")
	public String login(@RequestParam("emph") String emph,
			...
			try {
				...
				if (recruiter == null && jobSeeker == null) {
					...
					if (recruiter != null) {
						...
						String recruiterPassword = AES.decrypt(recruiter.getPassword());
						...
						String seekerPassword = AES.decrypt(jobSeeker.getPassword());
	...
	public String viewJobs(@RequestParam("query") String query,
			HttpSession session,
			ModelMap map) {

		List<Job> roleJobs =
				jobRepository.findByRoleLikeAndApprovedTrue("%" + query + "%");

		List<Job> skillJobs =
				jobRepository.findBySkillsLikeAndApprovedTrue("%" + query + "%");
